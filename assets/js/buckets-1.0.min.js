...
buckets.arrays.indexOf = function(a, b, c) {
    for (var c = c || buckets.defaultEquals,
    d = a.length,
    e = 0; e < d; e++) if (c(a[e], b)) return e;
    return - 1
};
buckets.arrays.lastIndexOf = function(a, b, c) {
    for (var c = c || buckets.defaultEquals,
    d = a.length - 1; 0 <= d; d--) if (c(a[d], b)) return d;
    return - 1
};
...
buckets.arrays.equals = function(a, b, c) {
    ...
    for (var d = a.length,
    e = 0; e < d; e++) if (!c(a[e], b[e])) return ! 1;
    return ! 0
};
...
buckets.LinkedList.prototype.add = function(a, b) {
    ...
    else if (0 === b) c.next = this.firstNode,
    this.firstNode = c;
    else {
...
buckets.LinkedList.prototype.remove = function(a, b) {
    ...
    for (var d = null,
    e = this.firstNode; null !== e;) {
        if (c(e.element, a)) return e === this.firstNode ? (this.firstNode = this.firstNode.next, e === this.lastNode && (this.lastNode = null)) : (e === this.lastNode && (this.lastNode = d), d.next = e.next, e.next = null),
        this.nElements--,
        !0;
        d = e;
        e = e.next
...
buckets.LinkedList.prototype.removeElementAtIndex = function(a) {
    if (! (0 > a || a >= this.nElements)) {
        var b;
        if (1 === this.nElements) b = this.firstNode.element,
        this.lastNode = this.firstNode = null;
        else if (a = this.nodeAtIndex(a - 1), null === a ? (b = this.firstNode.element, this.firstNode = this.firstNode.next) : a.next === this.lastNode && (b = this.lastNode.element, this.lastNode = a), null !== a) b = a.next.element,
        a.next = a.next.next;
        this.nElements--;
        return b
    }
...
buckets.LinkedList.prototype.toArray = function() {
    for (var a = [], b = this.firstNode; null !== b;) a.push(b.element),
    b = b.next;
    return a
};
...
buckets.Dictionary.prototype.remove = function(a) {
    var a = this.toStr(a),
    b = this.table[a];
    if (!buckets.isUndefined(b)) return delete this.table[a],
    this.nElements--,
    b.value
};
...
buckets.Dictionary.prototype.forEach = function(a) {
    for (var b in this.table) if (this.table.hasOwnProperty(b)) {
        var c = this.table[b];
...
buckets.MultiDictionary.prototype.set = function(a, b) {
    ...
    if (!this.containsKey(a)) return this.parent.set(a, [b]),
    !0;
...
buckets.Heap.prototype.siftUp = function(a) {
    for (var b = this.parentIndex(a); 0 < a && 0 < this.compare(this.data[b], this.data[a]);) buckets.arrays.swap(this.data, b, a),
    a = b,
    b = this.parentIndex(a)
};
buckets.Heap.prototype.siftDown = function(a) {
    for (var b = this.minIndex(this.leftChildIndex(a), this.rightChildIndex(a)); 0 <= b && 0 < this.compare(this.data[a], this.data[b]);) buckets.arrays.swap(this.data, b, a),
    a = b,
...
buckets.Heap.prototype.add = function(a) {
    if (!buckets.isUndefined(a)) return this.data.push(a),
...
buckets.BSTree.prototype.searchNode = function(a, b) {
    for (var c = null; null !== a && 0 !== c;) c = this.compare(b, a.element),
...
buckets.BSTree.prototype.successorNode = function(a) {
    ...
    for (var b = a.parent; null !== b && a === b.rightCh;) a = b,
    b = a.parent;
    return b
};
